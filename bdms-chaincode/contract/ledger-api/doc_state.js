...
/**
 * DocState class. States have a class, unique key, and a lifecycle current state
...
class DocState {
     ...
     */
    // 6개의 인자(상태, key, 파일제목, 파일확장자, 파일hash, IPFS주소)
    constructor(stateClass, keyParts, title, type, content, address) { // doc 생성자
        this.class = stateClass;
        this.key = DocState.makeKey(keyParts);
    ...
    getSplitKey(){
        return DocState.splitKey(this.key);
    }
    ...
    serialize() {
        return DocState.serialize(this);
    }
